  ...
  Future<String> executeCarService(int carId) async {
    ...
    try {
      final resp = await authDio.put(
        '${ApiConstants.executeCarService}$carId',
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[ExecuteCarServiceService] Success for carId: $carId');
        return resp.data?['message'] ?? 'Success';
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Execute car service failed: $message');
      }
    } on DioException catch (e) {
      log('[ExecuteCarServiceService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Execute car service failed: $message');
      } else {
        throw Exception('Execute car service failed: Şəbəkə xətası');
      }
    }
  }
