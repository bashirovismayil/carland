  ...
  Future<PolicyResponse> getPrivacyPolicy() async {
    ...
    try {
      final resp = await authDio.get(
        '${ApiConstants.privacyPolicy}$currentLanguage',
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[PrivacyPolicyService] Success');
        return PolicyResponse.fromJson(resp.data as Map<String, dynamic>);
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Get privacy policy failed: $message');
      }
    } on DioException catch (e) {
      log('[PrivacyPolicyService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Get privacy policy failed: $message');
      } else {
        throw Exception('Get privacy policy failed: Şəbəkə xətası');
      }
    }
  }
