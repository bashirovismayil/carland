...
class GetTransmissionTypeListService {
  final _local = locator<LoginLocalService>();
  final _languageService = locator<LanguageLocalService>();

  Future<List<GetTransmissionTypeListResponse>> getTransmissionTypeList() async {
    ...
    try {
      final resp = await authDio.get(
        ApiConstants.transmissionTypeList,
      ...
      if (resp.statusCode.isSuccess) {
        ...
        return data
            .map((item) => GetTransmissionTypeListResponse.fromJson(
            item as Map<String, dynamic>))
            .toList();
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Get transmission type list failed: $message');
      }
      ...
      if (e.response != null) {
        ...
        throw Exception('Get transmission type list failed: Şəbəkə xətası');
      }
    }
  }
