  ...
  Future<List<BrandListResponse>> getBrandList() async {
    ...
    try {
      final resp = await authDio.get(
        ApiConstants.getCarBrandList,
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[GetCarBrandListService] Success');
        if (resp.data is List) {
          return (resp.data as List)
              .map((item) =>
              BrandListResponse.fromJson(item as Map<String, dynamic>))
              .toList();
        }
        return [];
      } else {
      ...
      log('[GetCarBrandListService] DioException: $e');
      if (e.response != null) {
