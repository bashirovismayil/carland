...
class GetEngineTypeListService {
  final _local = locator<LoginLocalService>();
  final _languageService = locator<LanguageLocalService>();

  Future<List<GetEngineTypeListResponse>> getEngineTypeList() async {
    ...
    try {
      final resp = await authDio.get(
        ApiConstants.getEngineTypeList,
      ...
      if (resp.statusCode.isSuccess) {
        ...
        return data
            .map((item) => GetEngineTypeListResponse.fromJson(
            item as Map<String, dynamic>))
            .toList();
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Get engine type list failed: $message');
      }
      ...
      if (e.response != null) {
        ...
        throw Exception('Get engine type list failed: Şəbəkə xətası');
      }
    }
  }
