...
class GetCarRecordsService {
  final _local = locator<LoginLocalService>();
  final _languageService = locator<LanguageLocalService>();

  Future<List<GetCarRecordsResponse>> getCarRecords(String carId) async {
    ...
    try {
      final resp = await authDio.get(
        '${ApiConstants.getCarRecords}$carId',
      ...
      if (resp.statusCode.isSuccess) {
        ...
        return data
            .map((e) => GetCarRecordsResponse.fromJson(e as Map<String, dynamic>))
            .toList();
      } else {
      ...
      if (e.response != null) {
        ...
        throw Exception('Get car records failed: Şəbəkə xətası');
      }
    }
  }
