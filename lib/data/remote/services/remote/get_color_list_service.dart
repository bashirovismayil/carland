  ...
  Future<List<GetColorListResponse>> getColorList() async {
    ...
    try {
      ...
      if (resp.statusCode.isSuccess) {
        ...
        return data
            .map((json) =>
                GetColorListResponse.fromJson(json as Map<String, dynamic>))
            .toList();
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Get color list failed: $message');
      }
      ...
      if (e.response != null) {
        ...
        throw Exception('Get color list failed: Şəbəkə xətası');
      }
    }
  }
