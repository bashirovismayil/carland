  ...
  Future<DeleteAccountResponse> deleteAccount() async {
    ...
    try {
      ...
      if (resp.statusCode.isSuccess) {
        log('[DeleteAccountService] Account deleted successfully');
        return DeleteAccountResponse.fromJson(resp.data as Map<String, dynamic>);
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Delete account failed: $message');
      }
    } on DioException catch (e) {
      log('[DeleteAccountService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Delete account failed: $message');
      } else {
        throw Exception('Delete account failed: Şəbəkə xətası');
      }
    }
  }
