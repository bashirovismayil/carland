  ...
  Future<void> deleteNotification(int notificationId) async {
    ...
    try {
      ...
      if (resp.statusCode.isSuccess) {
        log('[DeleteNotificationService] Success for notificationId: $notificationId');
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Delete notification failed: $message');
      }
    } on DioException catch (e) {
      log('[DeleteNotificationService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Delete notification failed: $message');
      } else {
        throw Exception('Delete notification failed: Şəbəkə xətası');
      }
    }
  }
