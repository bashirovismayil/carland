  ...
  Future<UpdateCarRecordResponse> updateCarRecord({
    ...
    try {
      final resp = await authDio.put(
        ApiConstants.updateCarRecord,
        data: requestBody,
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        return UpdateCarRecordResponse(
          message: 'Record updated successfully',
          success: true,
        );
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Update car record failed: $message');
      }
      ...
      if (e.response != null) {
        ...
        throw Exception('Update car record failed: Şəbəkə xətası');
      }
    }
  }
