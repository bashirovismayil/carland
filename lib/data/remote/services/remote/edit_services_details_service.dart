  ...
  Future<EditCarServicesResponse> editCarServices({
    ...
    try {
      final resp = await authDio.put(
        ApiConstants.editCarServiceDetails,
        data: body,
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[EditCarServicesService] Success for carId: $carId');
        return EditCarServicesResponse.fromJson(resp.data as Map<String, dynamic>);
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Edit car services failed: $message');
      }
    } on DioException catch (e) {
      log('[EditCarServicesService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Edit car services failed: $message');
      } else {
        throw Exception('Edit car services failed: Şəbəkə xətası');
      }
    }
  }
