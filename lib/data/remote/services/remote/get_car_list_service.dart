  ...
  Future<List<GetCarListResponse>> getCarList() async {
    ...
    try {
      final resp = await authDio.get(
        ApiConstants.getCarList,
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[GetCarListService] Success: ${resp.data}');

        if (resp.data is List) {
          return (resp.data as List)
              .map((item) => GetCarListResponse.fromJson(item as Map<String, dynamic>))
              .toList();
        } else {
          return [];
        }
        ...
        throw Exception('Get car list failed: $message');
      }
    } on DioException catch (e) {
      log('[GetCarListService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Get car list failed: $message');
      } else {
        throw Exception('Get car list failed: Şəbəkə xətası');
      }
    }
  }
