...
class GetNotificationListService {
  ...
  Future<List<GetNotificationListResponse>> getNotificationList() async {
    ...
    try {
      final resp = await authDio.get(
        ApiConstants.getNotificationList,
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[GetNotificationListService] Success');

        if (resp.data is List) {
          return (resp.data as List)
              .map((item) =>
              GetNotificationListResponse.fromJson(item as Map<String, dynamic>))
              .toList();
        } else {
          return [];
        }
