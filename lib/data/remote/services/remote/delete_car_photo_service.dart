...
class DeleteCarPhotoService {
  ...
  Future<bool> deleteCarPhoto(int carId) async {
    ...
    try {
      final resp = await authDio.delete(
        '${ApiConstants.deleteCarPhoto}$carId',
        options: Options(headers: headers),
      );

      if (resp.statusCode.isSuccess) {
        log('[DeleteCarPhotoService] Success for carId: $carId');
        return true;
      } else {
        final message = _getErrorMessage(resp.statusCode ?? 0);
        throw Exception('Delete car photo failed: $message');
      }
    } on DioException catch (e) {
      log('[DeleteCarPhotoService] DioException: $e');
      if (e.response != null) {
        final message = _getErrorMessage(e.response?.statusCode ?? 0);
        throw Exception('Delete car photo failed: $message');
      } else {
        throw Exception('Delete car photo failed: Şəbəkə xətası');
      }
    }
  }
